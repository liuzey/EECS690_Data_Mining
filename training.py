...
def accuracy(logits, y):
    return torch.count_nonzero(y == (logits > 0.5))/len(y)


def TP(logits, y):
    p = torch.count_nonzero((logits > 0.5) * (y == (logits > 0.5)))
    return p


def TP_and_FP(logits, y):
    p1 = torch.count_nonzero(logits > 0.5)
    return p1


def TP_and_FN(logits, y):
    r1 = torch.count_nonzero(y >= 1)
    return r1
...
def pass_epoch(
    ...
    i_batch = -1
    for (img1, img2), y, (class1, class2) in loader:
        i_batch += 1
        if i_batch > LIMIT:
            break
        x1, x2 = img1.to(device), img2.to(device)
        y = y.to(device)
        y_pred = model(x1, x2)
